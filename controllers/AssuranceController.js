...
const getAssurances = async (req, res) => {
  try {
    const assurances = await AssuranceModel.find().sort({ createdAt: -1 })
...
const createAssurance = async (req, res) => {
  try {
    const { name, cons_price } = req.body
    const formErrors = []
    ...
    if(formErrors.length === 0) {
      await AssuranceModel.create(req.body)
...
const updateAssurance = async (req, res) => {
  try {
    ...
    if(formErrors.length === 0) {
      await AssuranceModel.updateOne({_id: id} , { name, cons_price }, { new: true })
      await getAssurances(req, res)
    } else {
      res.status(300).json({ formErrors })
    }
